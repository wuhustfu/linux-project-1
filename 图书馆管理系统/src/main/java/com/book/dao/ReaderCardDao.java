...
public class ReaderCardDao {
    ...
    private final static String FIND_ALL_ID_BY_USERID="select reader_id from reader_card WHERE reader_id = ?";
    ...
    private final static String IS_READER_EXISTED_SQL="SELECT reader_id FROM reader_card";
    ...
    public boolean AuthenReaderByReaderId(int userId){
        final ReaderCard readerCard=new ReaderCard();

        jdbcTemplate.query(FIND_ALL_ID_BY_USERID, new Object[]{userId},
                //匿名类实现的回调函数
                new RowCallbackHandler() {
                    public void processRow(ResultSet resultSet) throws SQLException {
                        readerCard.setReaderId(resultSet.getInt("reader_id"));
        ...
        if (readerCard.getReaderId() == ReaderCard.DEFAULT_ID)
            return false;
        return true;
    }
    ...
    public int addReaderCard(ReaderInfo readerInfo){
        ...
        if (IsReaderExisted(readerId))/**************changed*********************/
            return 0;
        return jdbcTemplate.update(ADD_READERCARD_SQL,new Object[]{readerId,name});
    }

    private boolean IsReaderExisted(int readerId) {
        final ArrayList<Integer> AllUserId = new ArrayList<Integer>();
        jdbcTemplate.query(IS_READER_EXISTED_SQL,
                //匿名类实现的回调函数
                new RowCallbackHandler() {
                    public void processRow(ResultSet resultSet) throws SQLException {
                        AllUserId.add(new Integer(resultSet.getInt("reader_id")));
                    }
                });
        if (AllUserId.contains(new Integer(readerId)))
            return true;
        return false;
    }
